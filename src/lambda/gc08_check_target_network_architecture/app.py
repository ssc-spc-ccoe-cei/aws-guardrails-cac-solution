...
def lambda_handler(event, context):
    ...
    if is_not_audit_account:
        logger.info(
            "Target Network Architecture document not checked in account %s - not the Audit account", aws_account_id
        )
        return
    ...
    if check_s3_object_exists(aws_s3_client, rule_parameters["s3ObjectPath"]):
        ...
        annotation = "Target Network Architecture document found"
    else:
        compliance_type = "NON_COMPLIANT"
        annotation = "Target Network Architecture document NOT found"
