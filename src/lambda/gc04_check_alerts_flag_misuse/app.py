...
def rule_matches_against_cb_role_identity(rule_event_pattern, cb_role_arn):
    ...
    return (
        cb_role_arn in ep_detail.get("userIdentity.sessionContext.sessionIssuer.arn", [])
...
def check_cb_role(cloud_trail_client, cb_role_name, cb_role_arn, event, aws_account_id):
    role_change_events = {
        "DeleteRolePolicy",
        "AttachRolePolicy",
        "DeleteRole",
    ...
    def do_lookup(attribute_value):
        ...
        while True:
            ...
            events_found.extend(response.get("Events", []))
            next_token = response.get("NextToken")
            if not next_token:
                break
        return events_found
    all_role_events.extend(do_lookup(cb_role_arn))
    all_role_events.extend(do_lookup(cb_role_name))
    for e in all_role_events:
        ...
        if event_name in role_change_events:
            return build_evaluation(
                ...
                event,
                annotation=(
                    f'Event "{e.get("EventId")}" was found performing action '
                    f'"{event_name}" on Cloud Broker role.'
                ),
            )
                ...
                if subscription.get("Protocol") == "email":
                    ...
                    if subscription_is_confirmed(sns_client, subscription_arn):
                        return build_evaluation(
                            ...
                            resource_type,
                            annotation=(
                                "An Event rule with SNS topic and subscription to send "
                                "notification emails is set up and confirmed."
                            ),
                        )
    return build_evaluation(
        rule.get("Name"),
        ...
        resource_type,
        annotation=(
            "An Event rule that has an SNS topic and subscription to send notification "
            "emails is not set up or confirmed."
        ),
    )
...
def lambda_handler(event, context):
    ...
    rule_parameters = check_required_parameters(
        ...
        ["ExecutionRoleName", "IAM_Role_Name"]
    )
    ...
    gr_requirement_type = check_guardrail_requirement_by_cloud_usage_profile(
        GuardrailType.Guardrail4,
        cloud_profile
    )
    ...
    cb_role_name = rule_parameters["IAM_Role_Name"]
    cb_role_arn = get_role_arn(aws_iam_client, cb_role_name)
    if not cb_role_arn:
        compliance_type = "NON_COMPLIANT"
        annotation = f"Cloud Broker Role with name '{cb_role_name}' not found."
    ...
    if not cb_rules:
        evaluations.append(
            build_evaluation(
                ...
                event,
                annotation=(
                    "No EventBridge rule found that alerts authorized personnel of misuse, "
                    "suspicious sign-in attempts, or changes to the CloudBroker role."
                ),
            )
        )
    else:
        for rule in cb_rules:
            ...
            rule_evaluation = check_rule_sns_target_is_setup(
                aws_sns_client,
                aws_event_bridge_client,
                rule,
                event
            )
        ...
        if rules_are_compliant:
            ...
            evaluations.append(
                build_evaluation(
                    ...
                    "NON_COMPLIANT",
                    event,
                    annotation="One or more EventBridge rules are not compliant.",
                )
            )
