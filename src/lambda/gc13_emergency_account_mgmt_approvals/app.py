    ...
    if is_scheduled_notification(invoking_event["messageType"]):
        ...
        if AWS_ACCOUNT_ID == AUDIT_ACCOUNT_ID:
            ...
            if check_s3_object_exists(valid_rule_parameters["s3ObjectPath"]):
                ...
                custom_annotation = "Emergency Account Management Procedure Approval found"
                logger.info('COMPLIANT - Emergency Account Management Procedure Approval found in S3 at "%s"', valid_rule_parameters["s3ObjectPath"])
            else:
                compliance_value = "NON_COMPLIANT"
                custom_annotation = "Emergency Account Management Procedure Approval NOT found"
                logger.info("NON_COMPLIANT - Emergency Account Management Procedure Approval not found in S3")
            ...
            logger.info("Emergency Account Management Procedure Approval not checked in account %s - not the Audit account", AWS_ACCOUNT_ID)
