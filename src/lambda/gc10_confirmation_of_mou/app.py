...
def get_assume_role_credentials(role_arn):
    """Gets the temporary credentials from the config service role.
    Keyword arguments:
...
def check_s3_object_exists(object_path):
    ...
    Keyword arguments:
    object_path -- the S3 object path to check
    """
    # parse the S3 path
...
def lambda_handler(event, context):
    ...
    It will be called by Lambda when the rule is triggered.
    """
    ...
    if is_scheduled_notification(invoking_event["messageType"]):
        ...
        if AWS_ACCOUNT_ID == AUDIT_ACCOUNT_ID:
            ...
            if check_s3_object_exists(valid_rule_parameters["s3ObjectPath"]):
                ...
                custom_annotation = "Signed MOU found"
                logger.info('COMPLIANT - Signed MOU found in S3 at "%s"', valid_rule_parameters["s3ObjectPath"])
            else:
                compliance_value = "NON_COMPLIANT"
                custom_annotation = "Signed MOU NOT found"
                logger.info("NON_COMPLIANT - Signed MOU not found in S3")
