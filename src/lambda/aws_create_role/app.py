...
def create_iam_policy(session, policy_name, policy_document):
    ...
    sts_response = iam_client.create_policy(PolicyName=policy_name, PolicyDocument=policy_document)
...
def delete_role(session, role_name):
    ...
    iam_client = session.client("iam")
    sts_response = iam_client.delete_role(RoleName=role_name)
    return sts_response
...
def assume_role(session, account_id, role_name):
    ...
    sts_response = sts_client.assume_role(
        RoleArn=f"arn:aws:iam::{account_id}:role/{role_name}", RoleSessionName=str(account_id)
    )
    ...
    try:
        ...
        for cfn_event in cfn_events:
            ...
            # Handle Delete
                ...
                logger.info(f"Unknown request type received: {request_type}")
                send(cfn_event, context, SUCCESS, response_data)
