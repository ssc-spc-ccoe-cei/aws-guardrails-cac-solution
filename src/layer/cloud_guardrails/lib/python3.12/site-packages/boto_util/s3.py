...
def list_all_s3_buckets(s3_client, page_size: int = 100, interval_between_calls: float = 0.05) -> list[dict]:
    args = {"MaxBuckets": page_size}
    resources: list[dict] = []

    while True:
        response = s3_client.list_buckets(**args)

        if response:
            resources.extend(response.get("Buckets", []))
            args["ContinuationToken"] = response.get("ContinuationToken")
        else:
            args["ContinuationToken"] = None

        if not args.get("ContinuationToken"):
            break
        else:
